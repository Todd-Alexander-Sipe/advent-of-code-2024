...
#   operator_index =>      This is going to be 3 to the power of the number of operators so index can be correct for loop
...
#   each "1" represents multiplication: "*", and each "2" represents smashing two strings together (example: "12" + "345" = "12345")
def check_a_problem(
    ...
    for i in range(operator_index):
        operators = ""
        problem = []
        current_binary_number = np.base_repr(i, base=3).zfill(number_of_operators)
        ...
        while len(problem) > 1:
            product_or_sum_or_concat = ""
            left_side = int(problem[0])
            operator = problem[1]
            right_side = int(problem[2])
            del problem[:3]
            if operator == "+":
                product_or_sum_or_concat = str(left_side + right_side)
            if operator == "*":
                product_or_sum_or_concat = str(left_side * right_side)
            if operator == "|":
                product_or_sum_or_concat = str(left_side) + str(right_side)
            problem.insert(0, product_or_sum_or_concat)
...
for input in inputs:
    ...
    operator_index = 3 ** (number_of_operators)
